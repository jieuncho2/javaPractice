...
그런데 어떤 인터페이스를 구현한 여러 클래스에서 사용할 메서드가 클래스마다 같은 기능을 제공하는 경우에
기능이 같다고 해도 인터페이스에거 코드를 구현할 수 없으므로 추상 메서드를 선언하고
각 클래스마다 똑같이 그 기능을 반복해서 구현해야 함

자바 8부터는 이런 부분에서 인터페이스 활용성을 높이기 위해 디폴트 메서드와 정적 메서드 기능을 제공
...
private 메서드는 인터페이스를 구현한 클래스에서 사용하게나 재정의할 수 없음
...
interface Calc { //인터페이스 만들기
	...
	/*
	private 메서드는 코드를 모두 구현해야 하므로 추상 메서드에는 private 예약어를 사용할 수 없음 
...
abstract class Calculator implements Calc { //인터페이스를 구현한 추상 클래스
	//추상 클래스의 경우 인터페이스의 모든 추상 메서드를 구현하지 않아도 됨
...
class CompleteCalc extends Calculator { //인터페이스를 구현한 추상 클래스를 상속받은 클래스
	//추상 클래스에서 add(), substract()을 구현했으니 times(), divide()만 구현하면 됨
