...
public class StudentManageB {
	private static StudentManageB singeltonManage;
	private StudentManageB() {}
	public static StudentManageB getInstance() {
		if(singeltonManage == null) {
			singeltonManage = new StudentManageB();
		}
		return singeltonManage;
	}
	private String strErr = "해당 학번의 학생은 존재하지 않습니다.";
	Scanner input = new Scanner(System.in);
	ArrayList<StudentB> students = new ArrayList();
	public static String[] className = { "JAVA", "C++", "C", "DATABASE" };

	StudentB findStudentInform(int studentNumber) {
		for (StudentB student : students) {
	...
	void addStudent() {
		...
		// 입력한 학번으로 새로운 학생을 생성
		StudentB newStudent = findStudentInform(studentNumber);
		if (newStudent == null) { //만약 기존에 학생의 정보가 없다면
			newStudent = new StudentB(studentNumber);
	...
	void setClass(String message, StudentB newStudent, int check) {
		...
		newStudent.setClassScore(index, 30);
		
		if(check == 1) { //수강 포기한 과목이라면
	...
	void Inform(int num) {
		...
		if(num == 1) {
			...
			// 입력한 학번으로 새로운 학생을 생성
			StudentB[] students = new StudentB[1];
